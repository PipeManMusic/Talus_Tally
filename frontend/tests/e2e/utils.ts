...
function resolveRepoRoot(): string {
  ...
  for (const candidate of candidates) {
    const templateCandidate = path.join(candidate, 'data', 'templates', `${E2E_TEMPLATE_ID}.yaml`);
    if (existsSync(templateCandidate)) {
      return candidate;
    }
  }
...
export async function openNewProjectDialog(page: Page) {
  const fileMenuButton = page.getByRole('button', { name: /^File$/i }).first();
  await expect(fileMenuButton).toBeVisible({ timeout: 10000 });
  await fileMenuButton.click();

  const newProjectMenuItem = page.getByRole('button', { name: /^New Project$/i }).first();
...
export async function openTemplateEditor(page: Page) {
  const toolsMenuButton = page.getByRole('button', { name: /^Tools$/i }).first();
  await expect(toolsMenuButton).toBeVisible({ timeout: 10000 });
  await toolsMenuButton.click();

  const templateEditorMenuItem = page.getByRole('button', { name: /^Template Editor$/i }).first();
